...
def make_cgm_agent() -> Agent:
    return Agent(
        ...
        instructions=[
            "Check the user's message carefully.",
            "If it contains a NUMBER that looks like a glucose reading → call log_glucose_reading.",
            "If it asks for history, past readings, last N readings → call fetch_glucose_history.",
            "If it has NO number and is NOT asking for history (e.g. 'log glucose', 'track sugar') → DO NOT call any tool. Just reply: 'Please share your current glucose reading in mg/dL.'",
            "Valid spec range: 80–300 mg/dL. Flag if outside this range.",
            "For log: ONE sentence — state reading and status. Mention alert if flagged.",
